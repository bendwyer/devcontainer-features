...
# Pass input from dev container feature into script
TARGET="${VERSION}"
...
# Move the binary to a central location
mv /root/.local/share/claude/versions/$version /usr/local/bin/claude
echo "Claude Code moved to /usr/local/bin/claude"
chmod +x /usr/local/bin/claude
chmod 0755 /usr/local/bin/claude
chown root:root /usr/local/bin/claude
echo "Claude Code permissions updated"
rm -r /root/.local/{bin,share,state}/claude
echo "Claude Code /root files cleaned up"
