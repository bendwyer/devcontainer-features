...
clean_download() {
    ...
    # try to use either wget or curl if one of them is already installed
    ...
    # alpine lacks bash, and RETURN is not a valid signal under sh shell
...
ensure_nanolayer() {
    ...
    # If previous installation not found, download it temporarily and delete it at the end of the script
    if [[ -z "${nanolayer_location}" ]]; then
        ...
        if [ "$(uname -sm)" == "Linux x86_64" ] || [ "$(uname -sm)" == "Linux aarch64" ]; then
            ...
            # clean download will minimize leftovers in case a downloader like wget or curl need to be installed
